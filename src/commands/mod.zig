...
fn cmd_help(sty: style.Style) !void {
    // Using std.debug.print for compatibility with Zig 0.15.1
    try sty.banner(undefined);
    std.debug.print("Usage: nen <group> <command> [args]\n\nGroups:\n  db      Graph database ops (status)\n  install Package management (install, list, update)\n  cache   (coming)\n  flow    (coming)\n\nExamples:\n  nen db status ./data\n  nen install nen-net\n  nen install nen-io --version v0.1.0\n\n", .{});
}
...
fn db_status(sty: style.Style, args: [][]const u8) !void {
    ...
    std.debug.print("Database functionality coming soon!\n", .{});
    std.debug.print("For now, use 'nen install' to manage packages.\n", .{});
}
...
fn db_init(sty: style.Style, args: [][]const u8) !void {
    ...
    std.debug.print("Database initialization coming soon!\n", .{});
    std.debug.print("For now, use 'nen install' to manage packages.\n", .{});
}
...
fn db_up(sty: style.Style, args: [][]const u8) !void {
    ...
    _ = args;
    // Using std.debug.print for compatibility with Zig 0.15.1
    std.debug.print("NenDB startup coming soon!\n", .{});
    std.debug.print("For now, use 'nen install' to manage packages.\n", .{});
}
...
fn db_snapshot(sty: style.Style, args: [][]const u8) !void {
    ...
    _ = args;
    // Using std.debug.print for compatibility with Zig 0.15.1
    std.debug.print("NenDB snapshots coming soon!\n", .{});
    std.debug.print("For now, use 'nen install' to manage packages.\n", .{});
}
...
fn db_restore(sty: style.Style, args: [][]const u8) !void {
    ...
    _ = args;
    // Using std.debug.print for compatibility with Zig 0.15.1
    std.debug.print("NenDB restore coming soon!\n", .{});
    std.debug.print("For now, use 'nen install' to manage packages.\n", .{});
}
...
fn install_package(sty: style.Style, args: [][]const u8) !void {
    if (args.len == 0) {
        ...
        std.debug.print("Usage: nen install <package-name> [--version <version>]\n", .{});
        std.debug.print("Available packages: nen-net, nen-io, nen-json, nenflow, nencache, nenff, nen-cli\n", .{});
        return;
    }
    ...
    const repo_url = get_repo_url(package_name) orelse {
        std.debug.print("Error: Unknown package '{s}'\n", .{package_name});
        std.debug.print("Available packages: nen-net, nen-io, nen-json, nenflow, nencache, nenff, nen-cli\n", .{});
        return;
    };
    ...
    std.debug.print("Package installed at: {s}\n", .{install_dir});
    std.debug.print("To use in your project, add to build.zig:\n", .{});
    std.debug.print("    const {s} = b.addModule(\"{s}\", .{{ .root_source_file = b.path(\"{s}/src/lib.zig\") }});\n", .{ package_name, package_name, install_dir });
}
...
fn list_packages(sty: style.Style) !void {
    ...
    for (packages) |pkg| {
        ...
        std.debug.print("{s:>12} Status: {s} | {s}\n", .{ "", pkg.status, pkg.github });
        std.debug.print("\n", .{});
    }
}
...
fn update_package(sty: style.Style, args: [][]const u8) !void {
    if (args.len == 0) {
        // Using std.debug.print for compatibility with Zig 0.15.1
        try sty.accent("Error: Package name required\n", undefined);
        std.debug.print("Usage: nen update <package-name>\n", .{});
        return;
    }
    ...
    if (!dir_exists) {
        std.debug.print("Error: Package '{s}' is not installed\n", .{package_name});
        std.debug.print("Use 'nen install {s}' to install it first\n", .{package_name});
        return;
    }
    ...
    std.debug.print("Successfully updated {s}!\n", .{package_name});
}
...
fn clone_repository(repo_url: []const u8, install_dir: []const u8) !void {
    ...
    // Using std.debug.print for compatibility with Zig 0.15.1
    std.debug.print("Would clone {s} to {s}\n", .{ repo_url, install_dir });
...
fn pull_latest_changes(install_dir: []const u8) !void {
    // Using std.debug.print for compatibility with Zig 0.15.1
    std.debug.print("Would pull latest changes for {s}\n", .{install_dir});
}

fn build_package(install_dir: []const u8) !void {
    // Using std.debug.print for compatibility with Zig 0.15.1
    std.debug.print("Would build package in {s}\n", .{install_dir});
